...
define([
  ...
  Handlebars.registerHelper("primitive", function (schema, options) {
    if (
      (schema.type && schema.type != "object" && schema.type != "array") ||
      schema.enum
    ) {
  ...
  Handlebars.registerHelper(
    "range",
    function (from, to, replFrom, replTo, exclFrom, exclTo, sep) {
      var result = "";
      if (from !== undefined || to !== undefined) {
        ...
        if ((from || replFrom) !== (to || replTo)) {
          result +=
            (from !== undefined || replFrom !== null) &&
            (to !== undefined || replTo !== null)
              ? sep
              : "";
  ...
  function renderSchema(schema) {
    if (stack.indexOf(schema) == -1) {
      ...
      return new Handlebars.SafeString(
        boxTemplate({ description: "_circular reference_" })
      );
    }
  }
    ...
    if (schema.$ref) {
      ...
      if (target) {
        ...
        result = new Handlebars.SafeString(
          "<span class='signature-type-ref'>" + schema.$ref + "</span>"
        );
      }
      if (target) {
        delete target.__ref;
  ...
  Handlebars.registerHelper("signature", function (schema, keyword, schemas) {
    ...
    return new Handlebars.SafeString(
      signatureTemplate({ schema: schema, keyword: keyword, schemas: schemas })
    );
  });
  ...
  function init() {
    $.when(
      ...
      $.get(docson.templateBaseUrl + "/signature.html").done(function (
        content
      ) {
        source = content;
